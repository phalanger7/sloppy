...
- 2026-09-02: Bot's spoken replies now render light blue in the TUI (was bold
  ...
  full last LLM call — system prompt, the messages verbatim (same format they
  were sent), the user prompt, and the output — accumulated in
  `get_last_llm_call()` (core, guarded by `_prompt_lock`) as the call happens;
  the modal is a `ModalScreen` pushed with `push_screen` (this Textual 8.2.8
  has no `sub_app`), closed via Esc, the X key, or a close button, and its
  text is written with `RichLog.write` (no `append`). The status pane
  advertises it with "press D to inspect last LLM call" (plus "press Q to
  quit"); 'q'/'Q' quit. The handler is `action_show_llm_debug` (Textual
  resolves a binding action to `action_<name>`); `action_show_llm_debug`,
  `on_button_pressed` and `border_title` are in the vulture ignore list
  (referenced only via the BINDINGS string / message dispatch / framework
  render). 200 tests, gate green. `bot.py` untouched.
...
- 2026-09-01: Stopped the bot talking about itself in the 3rd person. The LLM
  ...
  191 tests, gate green.
- 2026-08-31: TUI chatter (user) list now ordered by recency. `status_snapshot()`
  rendered `users` from registration order (`list(_users["names"])`); changed it
  to `_mention_targets_locked()` so the pane matches the order names go to the
  LLM (most recently spoken/engaged first). Extracted the lock-free core of
  `_mention_targets` into `_mention_targets_locked()` and made `_mention_targets`
  a thin locked wrapper, because `status_snapshot` already holds `_prompt_lock`
  (a non-reentrant `Lock`) and calling `_mention_targets()` under it deadlocked
  the TUI. Added `TestStatusSnapshotUsersOrder` (4 tests, asserts display ==
  mention order). 184 tests, gate green. `bot.py` untouched.
